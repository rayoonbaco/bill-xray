...
from engine.right_lens import build_candidate, build_right_lens
...
def test_right_candidate_is_always_explicit_interpretation():
    ...
    assert candidate.lens == "RIGHT"
    assert candidate.claim_class == "INTERPRETATION"
    assert candidate.status == "advocate_packet_ready"
    assert candidate.confidence >= 0.8


def test_health_domain_uses_conservative_health_questions_without_making_claim():
    candidate = build_candidate(
    ...
    assert any("choice" in q.lower() or "competition" in q.lower() for q in candidate.conservative_questions)
    assert "strongest good-faith conservative interpretation" in candidate.strongest_case_instruction
...
def test_right_lens_build_writes_section_packets(tmp_path, monkeypatch):
    import engine.right_lens as right
    ...
    for directory in (ingested_dir, segment_dir, anchor_dir, right_dir, topic_dir):
        directory.mkdir()
    ...
    monkeypatch.setattr(right, "ANCHOR_DIR", anchor_dir)
    monkeypatch.setattr(right, "RIGHT_DIR", right_dir)
    monkeypatch.setattr(right, "TOPIC_DIR", topic_dir)
    monkeypatch.setattr(right, "MONEY_DIR", tmp_path / "missing_money")
    monkeypatch.setattr(right, "POWER_DIR", tmp_path / "missing_power")
    monkeypatch.setattr(right, "BARREL_DIR", tmp_path / "missing_barrel")

    result = build_right_lens("demo")
    assert result.candidate_count == 2
    assert result.ready_count == 2
    assert all(item.claim_class == "INTERPRETATION" for item in result.candidates)
    assert (right_dir / "demo.json").exists()


def test_left_and_right_use_same_evidence_spine_but_distinct_frames():
    from engine.left_lens import build_candidate as build_left_candidate

    anchor = _anchor("SEC. 107. TAX CREDIT. A tax credit shall be allowed to the taxpayer.")
    topic = {
        "expert_domains": ["tax"],
        "primary_domain": "tax",
    ...
    left = build_left_candidate(anchor, topic_review=topic)
    right = build_candidate(anchor, topic_review=topic)
    ...
    assert left.evidence_snapshot == right.evidence_snapshot
    assert left.claim_class == right.claim_class == "INTERPRETATION"
    assert left.lens == "LEFT" and right.lens == "RIGHT"
    assert left.progressive_questions != right.conservative_questions
